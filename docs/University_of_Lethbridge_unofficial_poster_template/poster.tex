...
\begin{column}{\colwidth}
  ...
  \begin{block}{Analysis: When Memoization Helps}
    ...
    \begin{itemize}\setlength\itemsep{0pt}
      \item Pretzel and composite knots: denser crossing interactions, likely 40--50\%+ hit rates
      \item \texttt{std::unordered\_map} with custom hash: $O(1)$ average lookup instead of $O(\log n)$
      \item Lazy canonicalization: skip computing the canonical form unless actually storing
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Design Decisions \& Future Work}
    ...
    \textbf{Future work:}
    \begin{itemize}\setlength\itemsep{0pt}
      \item Profile with Valgrind/gprof to find where the overhead actually lives
